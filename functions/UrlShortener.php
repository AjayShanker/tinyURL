...
class UrlShortener {
    ...
    public function generateLinkForShortURL($uniqueCode = '') {
     ...
     /**
     * List of Tiny url is already present in database
     *
     * @param string $short
     *
     * @return boolean
     */
     ...
     public function tinyUrlData() {
        $data = array();
        $query  = $this->db->query("SELECT * FROM link ORDER BY created DESC");
        if ($query->num_rows) {
            $data = $query->fetch_all(MYSQLI_ASSOC);
        }
        $_SESSION['tiny_url_list'] = array();
        if (isset($_SESSION['tiny_url_list'])) {
            $_SESSION['tiny_url_list'] = $data;                    
        }
        return $data;
     }
}
